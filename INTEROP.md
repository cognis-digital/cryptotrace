...
How **cryptotrace** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `cryptotrace` lives in the **Sanctions - ownership - finance** cluster.
...
| from | relation | to |
|---|---|---|
| `cryptotrace` | JSON findings compose with | [`corpmap`](https://github.com/cognis-digital/corpmap), [`maritimeint`](https://github.com/cognis-digital/maritimeint) |
| `cryptotrace` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `cryptotrace` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `cryptotrace` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the sanctions - ownership - finance cluster.** `cryptotrace` output feeds the next tool:
```bash
cryptotrace ... --format json > out.json          # this tool's findings (see `cryptotrace --help`)
...
cryptotrace ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
cryptotrace ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
cryptotrace ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `cryptotrace`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `cryptotrace` slots
