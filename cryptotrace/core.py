...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'CT-SDN-001', 'severity': 'critical', 'weight': 3.0, 'title': 'OFAC_SDN_ADDRESS', 'when': {'sdn_listed': True}, 'remediation': 'Block; report per FinCEN guidance.'}, {'id': 'CT-MIXER-001', 'severity': 'high', 'weight': 2.5, 'title': 'MIXER_INTERACTION', 'when': {'mixer_hops': '^[1-9]'}, 'remediation': 'Document; review for funds-tracing purposes.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="crypto-trace",
                    metadata=item,
                ))
